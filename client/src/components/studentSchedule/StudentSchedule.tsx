...
const StudentSchedule = ({ match }: IStudentScheduleProps) => {
	...
	useEffect(() => {
		const dayIdx: ScheduleDay | undefined = decodeDay(params.dayIndex);
	...
	console.log("canGoBackInHistory", canGoBackInHistory);
				...
				<>
					...
					<StudentListModal
						...
						handleClose={() => {
							...
							// else {
							// 	const oldLocation: string = history.location.pathname;
							// 	const newLocation: string = `/${studentName}/${encodeDay(selectedDay)}`;
							...
							// if (history.location.pathname !== expectedLocation) {
							// 	history.push(expectedLocation);
							// }
