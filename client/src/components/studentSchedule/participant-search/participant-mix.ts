import { Class, Entries, Lesson, Participant, ParticipantLabel, Room, Student, Teacher, parseParticipants } from "@turbo-schedule/common";
...
export type ParticipantMix = Participant[] | GroupedParticipants | ParticipantLabelToTextToSnapshotObj | Lesson /** same as GroupedParticipants but with extras */;
...
export function parseParticipantMixIntoGroups(participants: ParticipantMix): ParticipantLabelToTextToSnapshotObj {
	const parsed: ParticipantLabelToTextToSnapshotObj = Array.isArray(participants)
		? Object.fromEntries(Object.entries(parseParticipants(participants)).filter(x => x[0] !== "allParticipants").map(([label, texts]) => [labelMultipleToSingleMap[label], Object.fromEntries(texts.map(t => [t, databaseFileName]))])) as ParticipantLabelToTextToSnapshotObj // TODO TS
