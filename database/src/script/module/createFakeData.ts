...
const pickNPseudoRandomly = (n: number) => <T>(origArr: T[], alreadyTakenArr: T[] = []): T[] => {
...
const randomUpToNExcl = (n: number): number => Math.ceil(Math.random() * (n - 1));

const pickSome = <T>(origArr: T[], alreadyTakenArr: T[] = []): T[] => {
